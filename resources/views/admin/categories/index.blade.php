...
<div class="mb-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            ...
            <p class="text-muted small mb-0">Tổ chức, phân cấp và quản lý các nhóm sản phẩm thời trang trong hệ thống BeeStyle</p>
        </div>
...
<div class="row g-3 mb-4">
  <div class="col-md-4">
    <div class="card border-0 shadow-sm rounded-3 p-3">
      <div class="d-flex align-items-center">
        ...
        <div>
          <div class="text-muted small fw-semibold">Tổng Số Danh Mục</div>
          <div class="fs-4 fw-bold text-dark">{{ $totalCategories ?? count($categories) }}</div>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="card border-0 shadow-sm rounded-3 p-3">
      <div class="d-flex align-items-center">
        ...
        <div>
          ...
          <div class="fs-4 fw-bold text-success">{{ $activeCategoriesCount ?? 0 }}</div>
        </div>
      </div>
    </div>
  </div>
  <div class="col-md-4">
    <div class="card border-0 shadow-sm rounded-3 p-3">
      <div class="d-flex align-items-center">
        <div class="avatar bg-info-subtle text-info rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px;">
          <i class="fa-solid fa-folder-tree fs-4"></i>
        </div>
        <div>
          <div class="text-muted small fw-semibold">Danh Mục Gốc (Root)</div>
          <div class="fs-4 fw-bold text-info">{{ $rootCategoriesCount ?? 0 }}</div>
        </div>
      </div>
    </div>
  </div>
...
<div class="bee-table-card">
    ...
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-3">
        <form action="{{ route('admin.categories.index') }}" method="GET" class="d-flex align-items-center gap-2 flex-wrap">
            <input type="text" name="q" value="{{ request('q') }}" class="form-control form-control-sm" placeholder="Tìm tên hoặc slug..." style="width: 250px;">
            ...
            @if(request('q') || (request('status') && request('status') !== 'all'))
                <a href="{{ route('admin.categories.index') }}" class="btn btn-sm btn-link text-danger p-0 ms-1">Xóa lọc</a>
            @endif
        </form>
        <div class="text-muted small">
            Hiển thị: <strong>{{ $categories->count() }}</strong> / <strong>{{ $categories->total() }}</strong> danh mục
        </div>
    </div>
        ...
        <table class="table align-middle mb-0">
            ...
            <tbody>
                @forelse($categories as $category)
                    <tr>
                        <td>
                            <strong class="font-monospace text-secondary">#{{ $category->id }}</strong>
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <span class="badge bg-warning-subtle text-dark border fw-bold">{{ $category->products_count }} SP</span>
                        </td>
                        <td>
                                ...
                                <button type="submit" class="btn btn-link p-0 border-0 text-decoration-none" title="Bấm để đổi trạng thái">
                                    ...
                                    @else
                                        <span class="badge bg-secondary-subtle text-secondary fw-bold">
                                            <i class="fa-solid fa-circle-pause me-1"></i> Tạm ẩn
                                        </span>
                                    @endif
                                </button>
                            </form>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="text-center py-4 text-muted">
                            ...
                            Chưa có danh mục thời trang nào phù hợp.
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
            <div class="modal-header border-bottom">
                <h5 class="modal-title fw-bold text-dark">
                    <i class="fa-solid fa-layer-group me-2 text-warning"></i> Thêm Danh Mục Thời Trang Mới
                </h5>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="sort_order" class="form-control" value="{{ $totalCategories + 1 }}" min="0">
                        </div>
                    </div>
                    <div class="mb-3">
