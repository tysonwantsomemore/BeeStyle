...
@section('title', 'Thanh Toán Thẻ MoMo Demo Sandbox — Đơn Hàng #' . $order->order_code)
  ...
  <div class="w-full max-w-4xl bg-white rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.06)] border border-neutral-200/80 overflow-hidden">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 min-h-[580px]">
      ...
      <div class="lg:col-span-5 p-6 sm:p-8 bg-[#fafbfc] border-b lg:border-b-0 lg:border-r border-neutral-200/80 flex flex-col justify-between space-y-6 text-xs">
        ...
        <div class="space-y-5">
          ...
          <div class="flex items-center gap-3 pb-4 border-b border-neutral-200/80">
            <div class="w-10 h-10 rounded-xl bg-[#a50064] text-white flex flex-col items-center justify-center font-black leading-none shadow-sm shrink-0">
              <span class="text-[10px] tracking-tighter">mo</span>
              <span class="text-[10px] tracking-tighter">mo</span>
            </div>
            <div>
              <span class="text-[10px] uppercase tracking-wider text-neutral-400 font-semibold block">Nhà cung cấp</span>
              <strong class="text-neutral-900 text-sm font-bold">MoMo Demo</strong>
            </div>
          </div>
          ...
          <div>
            ...
            <strong class="text-neutral-800 text-xs font-semibold block">
              Thanh toán đơn hàng #{{ $order->order_code }} qua MoMo
            </strong>
          </div>
      ...
      <div class="lg:col-span-7 p-6 sm:p-8 flex flex-col justify-between space-y-6">
        ...
        <!-- Thanh chọn nhanh tài khoản test MoMo Developer hỗ trợ -->
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <span class="text-[11px] font-bold uppercase tracking-wider text-neutral-600 flex items-center gap-1.5">
              ...
              Tài Khoản Test MoMo Developer Hỗ Trợ:
            </span>
            <span class="text-[10px] font-mono bg-pink-50 text-pink-700 px-2 py-0.5 rounded font-bold border border-pink-200">
              1-CLICK AUTOFILL
            </span>
          </div>
        ...
        <form id="momoCardForm" onsubmit="handleMomoSubmit(event)" class="space-y-4 text-xs">
        ...
        <div class="pt-3 border-t border-neutral-200/80 text-center space-y-2">
          <span class="text-[10px] uppercase tracking-wider text-neutral-400 font-semibold block">
            Ngân hàng chấp nhận thanh toán MoMo Sandbox
          </span>
...
<!-- ========================================================================= -->
<!-- MODAL XÁC THỰC MÃ OTP (MOMO DEVELOPER SANDBOX OTP MODAL) -->
<!-- ========================================================================= -->
<div id="momoOtpModal" class="fixed inset-0 z-50 bg-neutral-950/60 backdrop-blur-xs flex items-center justify-center p-4 hidden animate-fade-in">
  <div class="w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-neutral-200 space-y-5 text-center relative">
    ...
    <form action="{{ route('client.checkout.momo.success', $order->order_code) }}" method="POST">
      @csrf
    ...
    <form action="{{ route('client.checkout.momo.failed', $order->order_code) }}" method="POST">
      @csrf
      <button type="submit" class="w-full py-2.5 text-neutral-500 hover:text-rose-600 font-semibold text-xs transition-colors">
        Giả lập nhập sai OTP / Hủy giao dịch (ResultCode: 1006)
      </button>
    </form>

    <!-- Close button -->
    <button type="button" onclick="closeMomoOtpModal()" class="absolute top-4 right-4 text-neutral-400 hover:text-neutral-700">
...
<div id="safeGuideModal" class="fixed inset-0 z-50 bg-neutral-950/60 backdrop-blur-xs flex items-center justify-center p-4 hidden animate-fade-in">
  <div class="w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-neutral-200 space-y-4 text-xs relative">
    ...
    <div class="space-y-2.5 text-neutral-600 text-[11px] leading-relaxed">
      <p>• Đây là cổng thanh toán <strong>MoMo Developer Sandbox</strong> mô phỏng trực tiếp quy trình thanh toán thẻ ATM / Napas.</p>
...
@push('scripts')
<script>
  // Dữ liệu tài khoản test có sẵn của MoMo Developer Sandbox
  const TEST_CARDS = {
    saigonbank: {
      bank: 'Saigonbank',
  ...
  function handleMomoSubmit(e) {
    ...
    text.textContent = 'Đang kết nối MoMo...';
    btn.disabled = true;
    ...
    setTimeout(() => {
      ...
      // Open Modal
      document.getElementById('momoOtpModal').classList.remove('hidden');
    }, 600);
  }

  function closeMomoOtpModal() {
    document.getElementById('momoOtpModal').classList.add('hidden');
  }
