  ...
  <div class="bg-white rounded-2xl border border-neutral-200 shadow-xl overflow-hidden">
    <div class="bg-sky-900 text-white p-6 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-sky-800 flex items-center justify-center font-bold text-white">
          Z
        </div>
        <div>
          <h2 class="font-serif-luxury text-xl font-semibold">Thanh Toán Ví Điện Tử ZaloPay</h2>
          <p class="text-xs text-sky-200 font-light">Quét mã QR bằng ứng dụng Zalo / ZaloPay để thanh toán tức thì</p>
        </div>
      </div>
      <span class="text-xs text-sky-200 font-semibold flex items-center gap-1">
        <i data-lucide="shield-check" class="w-4 h-4"></i> Bảo mật ZaloPay
      </span>
    </div>

    <div class="p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center text-xs">
      <div class="space-y-4">
        <div class="p-5 bg-sky-50/50 rounded-xl border border-sky-200 space-y-3">
          ...
          <div class="flex justify-between">
            ...
            <strong class="text-sky-900 bg-sky-100 px-2 py-0.5 rounded font-mono">{{ $order->order_code }}</strong>
          </div>
        </div>
      ...
      <div class="text-center p-6 bg-neutral-50 rounded-xl border border-neutral-200 flex flex-col items-center">
        <span class="text-[11px] font-semibold uppercase tracking-wider text-neutral-600 mb-3">Mở ZaloPay quét mã thanh toán</span>
        <div class="bg-white p-3 rounded-xl border border-neutral-300 shadow-md">
          <img src="https://api.vietqr.io/image/970422-0987654321-compact2.jpg?amount={{ $order->total_amount }}&addInfo={{ $order->order_code }}&accountName=BEESTYLE%20ZALOPAY" alt="ZaloPay QR" class="w-56 h-56 object-contain">
        </div>
        <a href="{{ route('client.order-tracking', ['code' => $order->order_code]) }}" class="mt-4 px-6 py-2.5 bg-sky-700 text-white font-semibold uppercase tracking-wider text-xs rounded-lg hover:bg-sky-800 transition-colors">
